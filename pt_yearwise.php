				...
				$("#nextButton").click(function(){
					...
					$.post("pt_question_answer.php", dataToSend, function(data)
					{
							...
							if(pasedData.usercomment!= "")
						{
							...
							document.getElementById("question_id").innerHTML = pasedData.question_id;
                ...
                <div class="container-fluid">
                    <div class="row">
						...
						<h4 style="text-align:center">Question</h4>
							...
							<p id="question_id" hidden><?php echo $question_id;?></p>
						...
						<div class="col-sm-3" style="border: 1px solid grey; height: 400px; overflow: scroll; border-radius:3px">
							...
							<button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#thinking" aria-expanded="true" aria-controls="thinking">Click Here To Leave a Comment </button>
								<br/>
								<div class="collapse" id="thinking" style="margin-top:3px">
									<textarea class="form-control" rows="10" cols="30" id="myComment" placeholder="Please write how would you solve the question"></textarea><br/>
									<?php if(!empty($isLoginUser)){ ?>
										<button class="btn btn-info btn-sm" id="postComment" type="button"  aria-expanded="true" >Post comment </button>
									<?php  } else { ?> 
										<a class="btn btn-info btn-sm" type="button" href="login.php"  aria-expanded="true" >Login to Post comment </a>
									<?php }?>
								</div>
							<div id="ucomment">
							<?php
...
.comment {
    border-bottom: 1px solid rgba(16, 46, 46, 1);
    padding:8px;
   width: 100%;
    padding-left: 14px;
...
#ucomment{
    padding-bottom: 10px;
		...
		<script type="text/javascript">
			...
			var question_type = 'prelims_questions';
			getComments(question_id,question_type);
			$("#postComment").click(function(){
				
				var successMessage ='<span class="successMsg">Your Comment has been posted</span>';
				console.log('here');
				var comment = $('#myComment').val();
				 $.ajax({
		            type: "POST",
		            data: {comment:comment,question_id:question_id,question_type:question_type},
		            url: "/save_comment.php",
		            success: function(data){
		               	$(this).parent().parent().append(successMessage);
			...
			function formatComment(name,comment){
			    var commentHtml = '<div class="comment mt-3 text-justify float-left">\
                    <h4>'+name+'</h4>\
                    <p>'+comment+'</p>\
                </div>';
                return commentHtml;
			...
			function getComments(question_id,question_type){
				 $.ajax({
		            type: "GET",
		            data: {question_id:question_id,question_type:question_type},
		            url: "/comments.php",
		            success: function(data){
		     			...
		     			let html = '';
			               if(data['status']){
			               	for (var i = data.data.length - 1; i >= 0; i--) {
			               		...
			               		// html +="<br>"+element.user_name +"<br>"+ element.comment;
			               		html += formatComment(element.user_name,element.comment);
			               	}
			               
			               	document.getElementById("ucomment").innerHTML = html;
